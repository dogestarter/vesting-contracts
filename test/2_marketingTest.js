...
contract('MarketingVesting', (accounts) => {
  const [deployer, beneficiary1] = accounts;
  const initialReleasePercentage = new BN(10);
  const duration = time.duration.days(30*9);

  const allocatedTokens = new BN(web3.utils.toWei('20000000', 'ether'));
  ...
  beforeEach(async () => {
    this.start =  new BN((new Date("2022-09-22T16:00")).getTime()/1000);
    console.log(this.start.toNumber());
    this.vesting = await MarketingVesting.new(
      this.token.address,
      beneficiary1,
      this.start.toNumber(),
      allocatedTokens,
